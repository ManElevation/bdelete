minetest.register_on_placenode(function (pos, node, player)
	local file = io.open(minetest.get_modpath("bdelete").."/logs/"..player:get_player_name()..".log", "a")
...
local function bdelete(cmd, player, playername, time)
	if not minetest.get_player_privs(player)["privs"] then
		minetest.chat_send_player(player, "error: you don't have permission to bdelete")
		return
	end
	
	local file = io.open(minetest.get_modpath("bdelete").."/logs/"..playername..".log", "r")
	if file==nil then
	...
	file:close()
	local file = io.open(minetest.get_modpath("bdelete").."/logs/"..playername..".log", "w+")
	file:close()
end
...
minetest.register_on_chat_message(function(player, message)
	...
	if message:sub(0, #cmd) == cmd then
		...
		bdelete(cmd, player, playername, time)
		return true
	end	
	local cmd = "/bdelete"
	if message:sub(0, #cmd) == cmd then
		...
		bdelete(cmd, player, playername, nil)
		return true
	end
end)
